...
class Lattice(object):
    ...
    def __init__(self,N,beta,Nmatrix,epsilon, hits, N_measruements, N_corr,N_themral) -> None:
        ...
        self.N, self.N_x, self.N_t = N, N, N
        ...
        self.M=np.zeros((Nmatrix*2,2,2),dtype=complex)
        
        self.generte_random_matrix()

        self.U = np.zeros((self.N_t,self.N_x,4,2,2),dtype=complex) 
    ...
    def generte_random_matrix(self):
        ...
        for s in range(self.Nmatrix):
            ...
            self.M[s] = self.M[s]/np.linalg.det(self.M[s])**(1/2) #Unitarization

            self.M[s + self.Nmatrix] = Lattice.dagger(self.M[s]) #Saving the inverse

    def calculate_gamma(self, position_link):

        t, x, ro, N = position_link[0], position_link[1],  position_link[2], self.N

        gamma = 0. #inizializing gamma
        
        inc_ro = np.zeros((2), 'int')
        
        inc_ni = np.zeros((2), 'int')
        
        position = np.array([t, x])
        ...
        for ni in range(0, 2):
            ...
            if ni != ro :
                ...
                gamma+= np.dot(np.dot(self.U[tuple(position_one)+(ni,)],Lattice.dagger(self.U[tuple(position_two)+(ro,)])),Lattice.dagger(self.U[t,x,ni])) \
                        \
            ...
            for x in range(self.N_x):
                ...
                for u in range(2):
                    ...
                    for hit in range(self.hits):
                        ...
                        Delta_U = np.dot(self.M[s].copy(),self.U[t,x,u].copy())

                        Delta_S = -self.beta/(2)*np.real(np.trace(np.dot((np.dot(self.M[s].copy(),self.U[t,x,u].copy())-self.U[t,x,u].copy()),Gamma.copy())))

                        if Delta_S <0 or np.exp(-Delta_S) > np.random.uniform(0,1):

                            self.U[t,x,u] = Delta_U.copy()
    ...
    def generate_configurations(self):
        ...
        return(results)
    @staticmethod
    def get_plaquette(t,x, U):

        N, WL =len(U), 0

        

        incmi, incni =np.zeros((2),'int'), np.zeros((2),'int')
    ...
    def average_plaquette(lattice):
        ...
        for t in range(N):
            ...
            for x in range(N):
                ...
                result += Lattice.get_plaquette(t, x, lattice)
